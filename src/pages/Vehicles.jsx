...
import { Plus, Search, ChevronRight, Loader2, Truck } from 'lucide-react'
...
import {
  ...
  createVehicle, getLatestAssignments, getLatestMileage, getDriverById
...
export default function Vehicles() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.error(t('vehicles.addError'))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-5 sm:p-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {filtered.length > 0 ? (
          <>
                    ...
                    return (
                      <tr key={v.id} className="hover:bg-white transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <Link to={`/Vehicles/${v.id}`} className="text-slate-400 hover:text-[#1D4ED8]"><ChevronRight className="w-4 h-4 inline" /></Link>
                        </td>
                      </tr>
                    )
                  })}
            ...
            <div className="sm:hidden divide-y divide-slate-100">
              {filtered.map(v => {
                ...
                const mileage = latestMileage[v.id]
                return (
                  <Link key={v.id} to={`/Vehicles/${v.id}`} className="flex items-center justify-between p-4 hover:bg-white transition-colors">
          ...
          <EmptyState
            icon={Truck}
            title={search ? 'Aucun véhicule ne correspond' : 'Aucun véhicule'}
            description={search ? 'Essayez une autre plaque ou un autre modèle.' : 'Ajoutez votre premier véhicule à la flotte.'}
            action={!search ? { label: 'Ajouter un véhicule', onClick: () => setShowAdd(true) } : undefined}
          />
        )}
      </div>
