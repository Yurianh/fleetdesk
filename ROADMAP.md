...
- [x] **T-D19** — Palette d'urgence apaisée dans l'app : plus aucun rouge. Un seul bloc coloré — ambre — porte l'urgence réelle (dépassé en ambre plein, sous 7 jours en ambre clair) ; « à surveiller » et « documents à renouveler » passent au **gris graphite**, et l'émeraude reste pour ce qui est en règle. Le bleu de marque a d'abord servi au niveau « à surveiller », puis a été retiré : emprunté comme statut, il uniformisait l'écran et lui donnait l'air d'un gabarit. Le bleu reste le bleu de l'action. _(2026-09-15)_
- [x] **T-D20** — ~~Titres composés lettre par lettre~~ — **retiré** le 2026-09-15 : l'effet était demandé pour les réponses des questions fréquentes (T-D22), pas pour les titres de section. Les titres retrouvent l'arrivée sobre de la chorégraphie générale. _(2026-09-15)_
...
- [x] **T-L01** — Acceptation des conditions à l'inscription : case **décochée par défaut**, bouton bloqué tant qu'elle ne l'est pas, liens vers les CGU et la politique de confidentialité. L'acceptation est horodatée et versionnée dans les métadonnées du compte (`terms_accepted_at`, `terms_version`) — une case sans trace ne prouve rien. Distinction respectée : on **coche** pour les CGU (acceptation contractuelle), on **informe** pour les données (la base légale est le contrat, pas le consentement). _(2026-09-15)_
- [x] **T-L02** — Mentions au point de collecte côté site : rappel avant le bouton de création de compte sur `/souscrire/*`, et information RGPD sous le formulaire de contact, qui collectait nom, email et téléphone sans un mot. Année du pied de page de connexion corrigée (figée à 2025). _(2026-09-15)_
- [x] **T-V04** — L'alerte « Documents à renouveler » mène désormais **au document**, pas seulement au conducteur : le lien porte `?doc=<type>`, la fiche fait défiler jusqu'à la ligne concernée et la souligne quelques secondes. Sans ça, on atterrissait sur sept lignes sans savoir laquelle avait déclenché l'alerte. _(2026-09-15)_
- [x] **T-V05** — Jeu de démonstration : clés de documents corrigées (`visite_medecin`, `permis_conduire`, `formation_sst_psc1`…). Les valeurs inventées (`medical`, `permis`, `sst`) n'existaient pas dans `DOC_TYPE_CONFIG` : les documents étaient bien en base mais n'apparaissaient sur aucune ligne de la fiche (0/7 conformes) et l'alerte affichait la clé brute. Sophie Renard reçoit cinq documents pour que sa fiche tienne à l'écran. _(2026-09-15)_
- [x] **T-V06** — Un document dont le type ne correspond à aucune ligne réglementaire n'est plus invisible. La fiche ne montrait que les sept lignes connues plus les documents `autre:` : tout le reste était écarté silencieusement, d'où « 0/7 conformes » sur une fiche qui portait des documents. Les clés historiques (`medical`, `permis`, `sst`…) sont rattachées à la lecture, et un type inconnu obtient malgré tout sa ligne. `supabase/driver_documents_normalize_types.sql` remet la base au propre. _(2026-09-15)_
- [x] **T-V07** — Fiche conducteur et liste des conducteurs passées à la palette du tableau de bord : plus de rouge sur un document qui expire, mais l'échelle ambre graduée déjà en place sur les véhicules (expiré ambre-100, ≤30j ambre-50, ≤90j graphite, au-delà émeraude). Le rouge ne reste que sur la suppression. Les boutons « Mettre à jour » et « Renouveler » repassent en bleu : ce sont des actions, l'urgence est portée par la pastille. _(2026-09-15)_
- [x] **T-V08** — Jeu de démonstration complété : aucune fiche ne s'ouvre plus sur du vide. Contrôle technique, entretien réalisé et planning pour **les 15 véhicules** (contre 13, 6 et 6), 4 relevés kilométriques chacun, 2 lavages par véhicule affecté **par son propre conducteur**, et les **7 documents réglementaires pour les 12 conducteurs** (contre 4 conducteurs partiellement servis). La flotte est décrite une seule fois dans une table temporaire dont tout le reste dérive : les listes ne peuvent plus diverger d'une retouche à l'autre. Échéances toujours dosées — les documents sont étalés entre 60 jours et la fin de validité, donc les seules alertes sont celles de la démonstration : CT à 5 jours, CT à 22 jours, visite médicale de Sophie Renard à 21 jours, casier de Pierre Dubois expiré depuis 8 jours (les deux niveaux d'alerte conducteur). _(2026-09-15)_
- [x] **T-V09** — Fiches conducteurs renseignées de bout en bout : matricule, adresse, carte DKV, badge autoroute et carte lavage pour les douze. L'en-tête de la fiche s'ouvrait sur trois « Non renseigné » alignés, ce qui donne l'impression d'un compte à moitié créé. _(2026-09-15)_
- [x] **T-V12** — Script de démonstration réécrit en tonalité opérateur, puis détendu. L'ancienne version enchaînait les antithèses équilibrées, les triplets et les chutes aphoristiques — une voix qui s'entend comme générée. Le premier correctif est tombé dans l'excès inverse, une suite de phrases de trois mots qui sonne télégramme. Registre final : longueurs variées, connecteurs parlés (« bon », « donc », « alors »), une reprise assumée en cours de phrase, des raisons métier derrière les gestes (le rendez-vous de CT à prendre avant vendredi, le véhicule qui ne tourne pas), et les répliques annoncées comme des appuis à dire avec ses mots, pas un texte à réciter. L'ancienne version enchaînait les antithèses équilibrées, les triplets et les chutes aphoristiques — une voix qui s'entend comme générée. Nouveau registre : vocabulaire du métier (« le CT », « chauffeur », « la carte grise », « des bornes »), phrases courtes, chiffres dits à voix haute, silences notés comme des plans à part entière, une limite assumée à l'écran (les rapports sont mensuels), et une seule phrase de conclusion au lieu d'une par section. _(2026-09-15)_
- [x] **T-V11** — Script de démonstration relu contre l'application : durée réelle de la chorégraphie (4 s et non 2,5), chiffres du graphe recalculés après le nouveau seed (Ducato 12 600 km et non 24 343), séquence d'alerte déplacée sur le casier de Pierre Dubois — document à validité d'un an, donc l'échéance se calcule à l'écran, ce que la visite médicale de Sophie ne fait pas —, fiche véhicule décrite par ses onglets réels, écran Rapports rendu mensuel avec ses trois catégories de dépense, cadence de l'email d'échéances corrigée (hebdomadaire, resserrée sous 7 jours, pas quotidienne), page Lavages non filtrée par mois. _(2026-09-15)_
- [x] **T-V10** — Script de démonstration (`DEMO_SCRIPT.md`) : 4 min 30 minutées, ouverture sur le tableau de bord, narration et gestes séparés, checklist d'avant-tournage, liste explicite de ce qu'il ne faut pas montrer (dédoublonnage des CT, véhicules sans pièces jointes, rechargement qui tue la chorégraphie) et quatre accroches sectorielles. _(2026-09-15)_
- [x] **T-U09** — Le montant du plein devient obligatoire pour les chauffeurs, au même titre que les deux photos : un relevé saisi à la pompe sans montant oblige le gestionnaire à rouvrir le ticket pour le retrouver. Astérisque rouge, garde à l'enregistrement, et l'encadré bleu l'annonce avec les photos. Côté gestionnaire le champ reste optionnel — il rattrape souvent un kilométrage sans facture sous les yeux. _(2026-09-17)_
- [x] **T-U08** — Le tableau de bord ne clignote plus au premier chargement. Il est rendu **derrière** le voile de chargement et restait visible pendant les 350 ms de sa disparition : on le voyait donc au complet, puis disparaître d'un coup quand la chorégraphie démarrait. Une classe `is-landing-armed`, posée dès le montage avant toute peinture, masque les blocs `[data-land]` ; au lancement, elle cède la place à `is-landing` dans la même image. `useLayoutEffect` au lieu de `useEffect` pour que la bascule précède la peinture, et un filet à 6 s qui retire le masque si la séquence ne démarre jamais. _(2026-09-16)_
- [x] **T-U06** — Le gros lien « Ouvrir la fiche » quitte le dossier. La fiche s'ouvre d'un **clic sur le nom** — dans la liste ou dans l'en-tête du dossier — ou d'un **double-clic** sur la ligne, avec un rappel en 11 px puisqu'un double-clic ne se devine pas dans une application web. La ligne passe de `<button>` à `<div role="button">` : un lien ne peut pas vivre dans un bouton. _(2026-09-16)_
- [x] **T-U07** — Chorégraphie d'arrivée sur le dossier conducteur, rejouée à chaque sélection (`key` sur le panneau, `[data-panel-land]` dans `index.css`) : quatre blocs en cascade, puis les sept lignes de documents une à une. 460 ms contre 1 500 pour la séquence de connexion — celle-ci se rejoue plusieurs fois par minute, elle doit dire que le contenu a changé sans faire attendre. Même verrou `motion-on` que le reste. _(2026-09-16)_
- [x] **T-U04** — Liste des conducteurs en deux volets : la liste choisit, le dossier s'ouvre à côté. La pastille générique « Doc. expiré », identique pour quatre conducteurs sur six, est remplacée par le document et son retard — « Visite médicale expirée depuis 12 j ». Le dossier porte l'identité, le **véhicule actuel avec Désaffecter et Changer**, et les sept lignes réglementaires avec leur état ; l'historique des affectations, l'adresse et les numéros de carte restent sur la fiche, selon la ligne de partage « on décide ici, on étudie là ». Les trois colonnes de cartes et badges quittent le tableau : dix-huit cellules vides pour un tiers de la largeur. Tri par conformité par défaut. La suppression passe du bout de chaque ligne au bas du dossier, en gris — elle était à égalité visuelle avec le chevron d'ouverture. _(2026-09-16)_
- [x] **T-U05** — `AssignDriverDialog` accepte un `driverId` fixe : il ne savait affecter qu'un conducteur à un véhicule, il sait maintenant affecter un véhicule à un conducteur. Le sélecteur indique quels véhicules sont libres et qui conduit les autres, et prévient qu'en choisir un occupé échange les deux affectations. _(2026-09-16)_
- [x] **T-U01** — Fiche véhicule en deux colonnes. À gauche, fixe au défilement : l'identité, **« Ce qui tombe »** (contrôle technique, révision, documents) et les pièces jointes avec leur état. À droite, les cinq sections empilées — les onglets masquaient quatre blocs sur cinq alors qu'on ouvre justement cette page pour croiser le CT, la révision et le kilométrage. Les liens `?tab=` existants continuent d'amener au bon endroit, en faisant défiler. Largeur portée à 1 180 px : les tableaux s'arrêtaient à 1 024 px sur des écrans de 1 440. _(2026-09-16)_
- [x] **T-U02** — Liste des véhicules : deux densités, **Liste** (défaut, inchangée) et **Cartes**. La carte porte une phrase — « Contrôle technique dans 8 jours » — calculée depuis l'échéance la plus proche entre CT et révision, avec un axe de trois mois réduit à 10 px qui dit l'ordre et le nombre, pas la date. Le choix est mémorisé par navigateur (`src/lib/viewPreference.js`, clé `fd-view-vehicles`) : une densité appartient à l'écran qu'on a sous les yeux, pas au compte. Maquettes de travail conservées dans `prototypes/`. _(2026-09-16)_
- [x] **T-U03** — Jeu de démonstration intégré (`VITE_DEMO_MODE=true`) réparé : les contrôles techniques y étaient écrits `expiry_date` alors que l'application lit `expiration_date`, les plannings n'avaient pas d'intervalle en mois et les véhicules pas de date de mise en circulation. Aucun CT n'existait donc en mode démo. En développement, un visiteur non connecté arrive désormais sur `/login` au lieu d'être expédié sur fleetdesk.fr, ce qui rendait le serveur local inutilisable. _(2026-09-16)_
